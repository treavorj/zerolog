...
func TestLogStackFromContext(t *testing.T) {
	...
	got := out.String()
	want := `\{"stack":\[\{"func":"TestLogStackFromContext","line":"54","source":"stacktrace_test.go"\},.*\],"error":"from error: error message"\}\n`
	if ok, _ := regexp.MatchString(want, got); !ok {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
...
func TestLogStackFromContextWith(t *testing.T) {
	...
	got := out.String()
	want := `\{"level":"error","stack":\[\{"func":"TestLogStackFromContextWith","line":"67","source":"stacktrace_test.go"\},.*\],"error":"from error: error message"\}\n`
	if ok, _ := regexp.MatchString(want, got); !ok {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
